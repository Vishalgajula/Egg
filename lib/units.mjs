...
export function toMinor(rupees, what = 'amount') {
  // A missing value here almost always means stored data in an unexpected
  // shape rather than a typo in a form, so say which value was wrong.
  if (rupees == null)
    throw new Error(`No ${what} was found where one was expected.`);
  if (typeof rupees !== 'number' || !Number.isFinite(rupees) || rupees < 0)
    throw new Error(`Enter a valid ${what}.`);
